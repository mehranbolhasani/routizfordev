...
class Helper {
   ...
   public function json_decode($string, $assoc = false) {
    if (is_array($string) || $string === null) {
        return $string;
    }

    // Replace '\r\n' with '__NEW_LINE__' before decoding
    $string = str_replace('\r\n', '__NEW_LINE__', $string);
    ...
    public function pagination( $args = [] ) {
        ...
        if( isset( $url_parts[1] ) ) {
            ...
            unset( $url_query_args['post_id'] );
            unset( $url_query_args['search_form_id'] );
    ...
    public function get_listing_statuses() {
		return apply_filters(
			'routiz_listing_statuses', [
                ...
                'pending_listing' => _x( 'Pending', 'post status', 'routiz' ),
    ...
    function is_explore() {
        ...
        if( isset( $_GET['post_id'] ) and ! empty( $_GET['post_id'] ) ) {
			$object = get_post( $_GET['post_id'], OBJECT );
		}else{
            $object = get_queried_object();
        }

        if( is_object( $object ) && isset( $object->ID ) && ( $object->ID == get_option('rz_page_explore') ) ) {
            return true;
        }
