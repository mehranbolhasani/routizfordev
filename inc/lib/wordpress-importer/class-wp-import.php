 ...
 */
class WP_RZ_Import extends WP_Importer {
	...
	function import( $file ) {
		...
		$this->routiz_fix_urls();
		$this->routiz_fix_elementor_urls();
...
<?php
				...
				if( 'attachment' == $postdata['post_type'] ) {
					...
					if( isset( $post['postmeta'] ) ) {
			            foreach( $post['postmeta'] as $postmeta ) {
			                if( $postmeta['key'] == '_wp_attachment_metadata' ) {
			                    $attach_metadata = maybe_unserialize( $postmeta['value'] );
			                    if( isset( $attach_metadata['width'] ) and isset( $attach_metadata['height'] ) ) {
									// $image_width = $attach_metadata['width'];
									// $image_height = $attach_metadata['height'];
									$image_width = 1920;
									$image_height = 1080;
									// change remove url with placeholder
									$remote_url = "https://via.placeholder.com/{$image_width}x{$image_height}/222/333.jpg";
			                    }
			                    break;
			                }
			            }
					...
					$attach_args = [
						...
						'meta_query' => [
							'relation' => 'AND',
							[
								'key' => 'rz_imported_attachment',
								'value' => true,
								'compare' => '=',
							],
							[
								'key' => 'rz_attachment_size',
								'value' => "{$image_width}x{$image_height}",
								'compare' => '=',
							],
						],
					];

					$attach_query = new WP_Query( $attach_args );

					// use existing attachment with same size
					if( $attach_query->found_posts ) {
						$comment_post_ID = $post_id = $attach_query->posts[0]->ID;
						update_option('rz_import_attachment_id', $post_id);
					}
					// import the attachment
						...
						if( $image_width and $image_height ) {
							add_post_meta( $post_id, 'rz_attachment_size', "{$image_width}x{$image_height}" );
						}

						// set flag for imported attachment
						if( ! is_wp_error( $post_id ) ) {
							add_post_meta( $post_id, 'rz_imported_attachment', true );
						}
	...
	function routiz_fix_urls() {
		...
		if( $attach_query->found_posts ) {
		    $attach_id = $attach_query->posts[0]->ID;
		    $attach_src = wp_get_attachment_image_src( $attach_id, 'original' );
		    if( isset( $attach_src[0] ) ) {
		        $placeholder_image = $attach_src[0];
		    }
		}
	...
	function routiz_fix_elementor_urls() {
		...
		if( $results and is_array( $results ) ) {
		    ...
		    foreach( $results as $res ) {
		        $new_value = preg_replace(
		            ...
		            str_replace( '/', '\/', $placeholder_image ),
		            $res->meta_value
		        );
		        update_post_meta( $res->post_id, '_elementor_data', $new_value );
		    }
		}
